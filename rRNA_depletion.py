...
def run(protocol: protocol_api.ProtocolContext):
    ...
    tc_mod.set_block_temperature(95, hold_time_seconds=120, block_max_volume=15)
    # ramp down to temp 22C
    tc_mod.set_block_temperature(22, hold_time_seconds=30, ramp_rate=0.1) # OPS OPS chaning the ramp rate can give problems!!!
    # run at 22C for 5min
    tc_mod.set_block_temperature(22, hold_time_seconds=300, block_max_volume=15)
    # open lid
    tc_mod.open_lid()
    ...
    for i in range(sample_nr):
        pipette_single.distribute(2, RNaseH_buffer, tc_plate.rows()[i])
    # add the RNaseH_enzyme buffer to each sample in thermocyler(start A1, pr row)
    for i in range(sample_nr):
        pipette_single.distribute(2, RNaseH_enzyme, tc_plate.rows()[i])
    # add Nuclease free water
    for i in range(sample_nr):
        pipette_single.distribute(1, nfH2O, tc_plate.rows()[i])
    ...
    # add the RNaseH_buffer buffer to each sample in thermocyler(start A1, pr row)
    for i in range(sample_nr):
        pipette_single.distribute(5, DNaseI_buffer, tc_plate.rows()[i])
    # add the RNaseH_enzyme buffer to each sample in thermocyler(start A1, pr row)
    for i in range(sample_nr):
        pipette_single.distribute(2.5, DNaseI_enzyme, tc_plate.rows()[i])
    # add Nuclease free water
    for i in range(sample_nr):
        pipette_single.distribute(22.5, nfH2O, tc_plate.rows()[i])
    ...
    pipette_multi.distribute(141, tc_plate.rows()[0], mag_plate.rows()[0])
    ...
    pipette_multi.distribute(145, mag_plate.rows()[0], liquid_waste)

    # add ethanol80 to magnetic plate.
    # THIS SHOULD BE DONE VERY SLOWLY FIND WAY TO SUCK VERY SLOWLY!!!
    pipette_multi.distribute(200, ethanol80, mag_plate.rows()[0])
    ...
    # remove liquid from magnetic plate.
    # THIS SHOULD BE DONE VERY SLOWLY FIND WAY TO SUCK VERY SLOWLY!!!
    pipette_multi.distribute(201, mag_plate.rows()[0], liquid_waste)
    ...
    for i in range(sample_nr):
        pipette_single.distribute(7, nfH2O, mag_plate.rows()[i])
